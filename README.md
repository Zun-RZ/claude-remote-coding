...
Run these **one at a time** — enter the first, wait for `Added marketplace remote-tabs`, then enter the second. Don't paste both lines in a single message (the second line gets swallowed into the first as a bad repo argument and the clone fails).
